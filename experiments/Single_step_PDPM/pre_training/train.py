...
best_val, stall, patience, num_epochs = np.inf, 0, 6, 150
...
for epoch in range(1, num_epochs + 1):
    ...
    if val_loss < best_val:
        ...
        torch.save(model.state_dict(), f"{ckpt_dir}/best_sspdpm_pre_train.pth")  # 
    else:
        stall += 1
...
torch.save(model.state_dict(),
           f"{ckpt_dir}/final_{datetime.now():%Y%m%d_%H%M%S}_sspdpm_pre_train.pth")
