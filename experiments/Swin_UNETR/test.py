...
model.load_state_dict(torch.load("models/swin_unetr_checkpoints/best_model_swin_unetr_xcat.pth", map_location=device))
model.eval()
...
if groups["1.0"]:
    ...
    with torch.no_grad():
        for batch in loader_ref:
            lbl = batch["label"]                 # [B,1,D,H,W]
            scale = batch["scale"].view(-1,1,1,1,1)
    ...
    for noise, file_list in groups.items():
        ...
        for i, batch in enumerate(loader):
            ...
            visualize_predictions(
                ...
                save_dir=f"./visuals_xcat_swin_unetr/{noise}",
